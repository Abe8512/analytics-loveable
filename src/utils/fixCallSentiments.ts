...
export const fixCallSentiments = async (): Promise<{
  ...
  try {
    ...
    const { data: calls, error } = await supabase
      ...
      .select('id, text, sentiment, call_score')
    ...
    if (calls) {
      for (const call of calls) {
        try {
          ...
          // Analyze the text to determine sentiment
          ...
          updated++;
          console.log(`Successfully updated call ${call.id} to ${sentiment} with score ${callScore}`);
        } catch (err) {
