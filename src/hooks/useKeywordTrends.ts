...
export const useKeywordTrends = (initialCategory: KeywordCategory = 'all') => {
  ...
  // Group keywords by category for easy access
  const keywordTrends = useMemo(() => {
    const result: Record<KeywordCategory, KeywordTrend[]> = {
      all: keywords,
      positive: keywords.filter(k => k.category === 'positive'),
      negative: keywords.filter(k => k.category === 'negative'),
      neutral: keywords.filter(k => k.category === 'neutral'),
      general: keywords.filter(k => k.category === 'general')
    };
    return result;
  }, [keywords]);
