...
// Max concurrent connections tracker
const pendingRequests = {
  count: 0,
  max: 10, // Maximum concurrent requests
  queue: [] as Function[],
  ...
  SUPABASE_PUBLISHABLE_KEY,
  {
    ...
    global: {
      fetch: async (...args: [RequestInfo | URL, RequestInit?]): Promise<Response> => {
        ...
        // Return a promise that will resolve when we can execute the request
        return new Promise<Response>((resolve, reject) => {
          const executeRequest = async () => {
            try {
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    if (responseText.includes("invalid input syntax for type uuid")) {
                      ...
                      pendingRequests.count--;
                      pendingRequests.processQueue();
                      reject(new Error(`UUID format error: ${responseText}`));
                      return;
                    }
                    
                    // Handle specific error status codes
                    if (response.status === 429) {
                      console.warn('Too many requests, applying backoff strategy');
              ...
              pendingRequests.count--;
              pendingRequests.processQueue();
              resolve(new Response(JSON.stringify({ error: 'Connection failed after retries' }), {
                status: 503,
              ...
              pendingRequests.count--;
              pendingRequests.processQueue();
              resolve(new Response(JSON.stringify({ error: 'Network request failed' }), {
                status: 503,
                headers: { 'Content-Type': 'application/json' }
              }));
            }
          ...
          if (pendingRequests.count < pendingRequests.max) {
            ...
            console.log('Too many concurrent requests, queueing');
